...
function App() {
  ...
  const getAdvice = useCallback(async () => {
    try {
      ...
      if (!history.includes(data.slip.advice)) {
        setHistory((prev) => [data.slip.advice, ...prev].slice(0, 5));
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-purple-00 to-blue-800 relative">
        ...
        {error ? (
          ...
          <>
            {isLoading ? (
              ...
              <h1 className="text-3xl text-white glow text-center mb-10">{advice}</h1>
            )}
          </>
        )}
